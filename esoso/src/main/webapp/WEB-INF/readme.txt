...
性能介绍：
	...
	5. 对与服务页面，暂时定死，只有改动的值是变化的。【待改进】
	6. 重新校验的存储过程的问题，改正！对于Website的默认设置，没有控制到type2。【待思考】
...
特色设计：
	...
	4. 在删除站点信息时，会删除站点信息，但并不会直接删除字节码文件等，而是通过定时调度器取数据库获取链接，扫描文件夹，进行清理
	(另一种方法，对删除的数据做个字段改变，不直接删除数据，再定时调度删除数据)
